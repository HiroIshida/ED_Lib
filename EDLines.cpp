...
void EDLines::ValidateLineSegments()
{
  ...
  for (int i = 0; i < linesNo; i++)
  {
    ...
    {
      // Longer line segments are first validated by a line support region rectangle having width=1
      // (for speed) If the line segment is still invalid, then a line support region rectangle
      // having width=2 is tried If the line segment fails both tests, it is discarded
      int aligned = 0;
      int count = 0;
...
//
void EDLines::ComputeClosestPoint(double x1, double y1, double a, double b, int invert,
...
void EDLines::SplitSegment2Lines(double *x, double *y, int noPixels, int segmentNo,
                                 vector<LineSegment> &lines, int min_line_len, double line_error)
{
  // First pixel of the line segment within the segment of points
  int firstPixelIndex = 0;
