...
#define CIRCLE_MIN_LINE_LEN 6
  ...
  for (int i = 0; i < segmentNos; i++)
  {
    ...
    // If the segment is reasonably long, then see if the segment traverses the boundary of a closed
    // shape
    if (noPixels >= 4 * CIRCLE_MIN_LINE_LEN)
    {
      // If the end-points of the segment is close to each other, then assume a circular/elliptic
      // structure
      ...
      if (d <= maxDistanceBetweenEndPoints)
      {
        ...
        else if (ellipseFitError <= ELLIPSE_ERROR)
        {
          ...
          if (major < 8 * minor)
          {
            addCircle(circles1, noCircles1, xc, yc, r, circleFitError, &eq, ellipseFitError, x, y,
                      noPixels);
            bm->move(noPixels);
          }  // end-if
...
cv::Mat EDCircles::drawResult(bool onImage, ImageStyle style)
{
  ...
  if (style == ImageStyle::ELLIPSES || style == ImageStyle::BOTH)
    ...
    {
      double degree = (ellipses[i].theta * 180) /
                      PI;  // convert radian to degree (opencv's ellipse function works with degree)
      ellipse(colorImage, ellipses[i].center, ellipses[i].axes, degree, 0.0, 360.0,
...
void EDCircles::GenerateCandidateCircles()
{
  ...
  for (int i = 0; i < edarcs4->noArcs; i++)
  {
    if (arcs[i].isEllipse)
    {
      ...
      {
        addCircle(circles1, noCircles1, arcs[i].xc, arcs[i].yc, arcs[i].r, arcs[i].circleFitError,
                  &arcs[i].eq, arcs[i].ellipseFitError, arcs[i].x, arcs[i].y, arcs[i].noPixels);
      }
      else
      {
        //        double coverRatio = arcs[i].coverRatio;
        double coverRatio =
            MAX(ArcLength(arcs[i].sTheta, arcs[i].eTheta) / TWOPI, arcs[i].coverRatio);
    ...
    else
    {
      ...
      {
        addCircle(circles1, noCircles1, arcs[i].xc, arcs[i].yc, arcs[i].r, arcs[i].circleFitError,
                  &eq, ellipseFitError, arcs[i].x, arcs[i].y, arcs[i].noPixels);
      }  // end-if
    }    // end-else
  }      // end-for
}
  ...
  for (int iter = 1; iter <= 2; iter++)
  {
    ...
    for (int curSegmentNo = 0; curSegmentNo < segmentNos; curSegmentNo++)
    {
      ...
      while (firstLine < stopLine - 1)
      {
        ...
        int wrapCase = -1;  // 1: wrap the first two lines with the last line, 2: wrap the last two
                            // lines with the first line
#if 0
        ...
        {
          // Just 2 lines. If long enough, then try to combine. Angle between 15 & 45 degrees. Min.
          // length = 40
          ...
          if (info[firstLine].angle >= PI / 12 && info[firstLine].angle <= PI / 4 &&
              totalLineLength >= 40 && shorterLen * 2 >= longerLen)
          {
            specialCase = true;
          }  // end-if

          // If the two lines do not make up for arc generation, then try to wrap the lines to the
          // first OR last line. There are two wrapper cases:
          if (specialCase == false)
          {
            // Case 1: Combine the first two lines with the last line of the segment
            if (firstLine == segmentStartLines[curSegmentNo] &&
                info[stopLine - 1].angle >= MIN_ANGLE && info[stopLine - 1].angle <= MAX_ANGLE)
            {
              wrapCase = 1;
              specialCase = true;
            }  // end-if
        ...
        if (circleFitError <= MYERROR)
        {
          ...
          else
          {
            ...
            addArc(edarcs1->arcs, edarcs1->noArcs, xc, yc, radius, circleFitError, sTheta, eTheta,
                   info[firstLine].sign, curSegmentNo, (int)x[0], (int)y[0], (int)x[noPixels - 1],
        ...
        // Check if this is an almost closed loop (i.e, if 60% of the circle is present). If so, try
        // to fit an ellipse to the entire arc of lines
        double dx = x[0] - x[noPixels - 1];
        double dy = y[0] - y[noPixels - 1];
        double distanceBetweenEndPoints = sqrt(dx * dx + dy * dy);

        bool isAlmostClosedLoop =
            (distanceBetweenEndPoints <= 1.72 * radius && coverage >= FULL_CIRCLE_RATIO);
        if (isAlmostClosedLoop || (iter == 1 && coverage >= 0.25))
        {  // an arc covering at least 90 degrees
          EllipseEquation eq;
          double ellipseFitError = 1e10;
          ...
          if (ellipseFitError <= MYERROR)
          {
            ...
            {
              addCircle(circles1, noCircles1, xc, yc, radius, circleFitError, &eq, ellipseFitError,
                        x, y, noPixels);  // Add an ellipse for validation
            }
            else
            {
              double sTheta, eTheta;
        ...
        while (firstLine <= lastLine - 2)
        {
          ...
          while (curLine <= lastLine)
          {
            ...
            if (error > LONG_ARC_ERROR)
            {
              ...
              break;
            }  // Adding this line made the error big. So, we do not use this line
          ...
          else
          {
            ...
            addArc(edarcs1->arcs, edarcs1->noArcs, XC, YC, R, Error, sTheta, eTheta,
                   info[firstLine].sign, curSegmentNo, (int)x[0], (int)y[0], (int)x[noPixels - 1],
                   (int)y[noPixels - 1], x, y, noPixels);
          }  // end-else
    ...
    for (int j = 0; j < noPoints; j++)
    {
      ...
      if (edgeImg[r * width + c] != 255)
      {
        ...
        continue;  // Ignore non-edge pixels.
                   // This produces less false positives, but occationally misses on some valid
                   // circles
      }            // end-if
    out:
...
void EDCircles::JoinCircles()
{
  ...
  for (int i = 0; i < noCircles; i++)
  {
    ...
    {
      ComputeEllipseCenterAndAxisLengths(&circles[i].eq, &circles[i].xc, &circles[i].yc,
...
#define CENTER_DISTANCE_THRESHOLD 12        //(AXIS_LENGTH_DIFF_THRESHOLD*2)
    ...
    {
      addCircle(circles3, noCircles3, XC, YC, R, CircleFitError, &Eq, EllipseFitError, NULL, NULL,
                0);
    }
    else
    {
...
void EDCircles::JoinArcs1()
{
  ...
  {
    int arcNo;
    int which;    // 1: (SX, SY)-(sx, sy), 2: (SX, SY)-(ex, ey), 3: (EX, EY)-(sx, sy), 4: (EX,
                  // EY)-(ex, ey)
  ...
  for (int i = 0; i < noArcs; i++)
  {
    ...
    else
    {
      ...
      if ((coverage >= FULL_CIRCLE_RATIO && CircleFitError <= LONG_ARC_ERROR))
        ...
        addArc(edarcs2->arcs, edarcs2->noArcs, XC, YC, R, CircleFitError, sTheta, eTheta, Turn,
               arcs[i].segmentNo, SX, SY, EX, EY, x, y, NoPixels, angles.overlapRatio());
  ...
  for (int i = 0; i < noArcs; i++)
  {
    ...
    else
    {
      ...
      if ((coverage >= FULL_CIRCLE_RATIO && CircleFitError <= LONG_ARC_ERROR))
        ...
        addArc(edarcs3->arcs, edarcs3->noArcs, XC, YC, R, CircleFitError, sTheta, eTheta, Turn,
               arcs[i].segmentNo, &Eq, EllipseFitError, SX, SY, EX, EY, x, y, NoPixels,
               angles.overlapRatio());
  ...
  for (int i = 0; i < noArcs; i++)
  {
    ...
    else
    {
      ...
      if ((coverage >= FULL_CIRCLE_RATIO && CircleFitError <= LONG_ARC_ERROR))
        ...
        addArc(edarcs4->arcs, edarcs4->noArcs, XC, YC, R, CircleFitError, sTheta, eTheta, Turn,
               arcs[i].segmentNo, &Eq, EllipseFitError, SX, SY, EX, EY, x, y, NoPixels,
               angles.overlapRatio());
...
void EDCircles::joinLastTwoArcs(MyArc *arcs, int &noArcs)
{
  ...
  if (circleFitError <= LONG_ARC_ERROR)
  {
    ...
    //    arcs[prev].eTheta = arcs[last].eTheta;   -- Fails in a very nasty way in a very special
    //    case (recall circles9 with cvsmooth(7x7)!) So, do not use.
...
//
void EDCircles::ComputeStartAndEndAngles(double xc, double yc, double r, double *x, double *y,
...
//
bool EDCircles::CircleFit(double *x, double *y, int N, double *pxc, double *pyc, double *pr,
                          double *pe)
{
  *pe = 1e20;
  if (N < 3) return false;
